...
namespace Knx.ObjectModel
{
	public class ObjectModelFactory
	{
		...
		public static Model BuildModel(ProjectStore store)
		{
			...
			var prog = ObjectModelFactory.BuildProgramming(store.Manufacturers.Select(row => row.Document), project);
			return new Model()
			{
				Project = project,
				Hardware = ObjectModelFactory.BuildHardware(store.Hardwares.Select(row => row.Document), project, prog, manus),
				Programs = prog,
				Types = types
			};
		}
